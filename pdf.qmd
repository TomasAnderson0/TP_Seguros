...
ggplot() + 
  ...
  geom_vline(data = datos, aes(xintercept  = mean(cuantia)), linewidth = .75, color = "darkgreen") +
...
```{r, echo=FALSE, fig.cap= "Función de densidad Weibull estimada", message=FALSE}
...
ggplot(data.frame(x = resultado)) +
  ...
  geom_vline(aes(xintercept  = quantile(x, probs = .99)), linewidth = .75, color = "darkgreen")  +
  geom_vline(aes(xintercept  = quantile(x, probs = .5)), linewidth = .75, color = "darkgreen")  +
  annotate("text",x = quantile(resultado, probs = .99) + 35, y = 1150, label = "Percentil 99%") +
    annotate("text",x = quantile(resultado, probs = .5) + 35, y = 1150, label = "Percentil 50%") +
  labs(x = "Millones de pesos (a precios de 2024)", y = "Frecuencia absoluta") +
  scale_x_continuous(breaks = seq(1500, 2300, 50)) +
  scale_y_continuous(limits = c(0, 1200))
```
 
La mediana del total anual de las cuantías de los siniestros simulados es `r round(median(resultado), 2)` millones de pesos. El valor que acumula el 99% del total de las cuantías posibles es de `r round(quantile(resultado, probs = .99), 2)` millones de pesos. Esto es un `r round(100 * (quantile(resultado, probs = .99)/quantile(resultado, probs = .5)-1), 2)`% más grande que la mediana.
...
ggplot(data.frame(x = resultado_sampling)) +
  geom_histogram(aes(x), color = "black", fill = "gold") +
  geom_vline(aes(xintercept  = quantile(x, probs = .99)), linewidth = .75, color = "darkgreen")  +
  geom_vline(aes(xintercept  = quantile(x, probs = .5)), linewidth = .75, color = "darkgreen")  +
  annotate("text",x = quantile(resultado_sampling, probs = .99) + 38, y = 1100, label = "Percentil 99%") +
    annotate("text",x = quantile(resultado_sampling, probs = .5) + 35, y = 1100, label = "Percentil 50%") +
  labs(x = "Millones de pesos (a precios de 2024)", y = "Frecuencia absoluta") +
  scale_x_continuous(breaks = seq(1500, 2300, 50)) +
  scale_y_continuous(limits = c(0, 1200))
```
...
La mediana del total anual de las cuantías de los siniestros simulados es `r round(median(resultado_sampling), 2)` millones de pesos. El valor que acumula el 99% del total de las cuantías posibles es de `r round(quantile(resultado_sampling, probs = .99), 2)` millones de pesos. Esto es un `r round(100 * (quantile(resultado_sampling, probs = .99)/quantile(resultado_sampling, probs = .5)-1),2)`% más grande que la mediana.
...
Metodo               & \multicolumn{1}{l|}{1\%} & \multicolumn{1}{l|}{2\%} & 4\% \\ \hline
Weibull              & \multicolumn{1}{l|}{`r round(1000 * quantile(resultado, probs = .5) * 1.01 / 25615, 3)`}   & \multicolumn{1}{l|}{`r round(1000 * quantile(resultado, probs = .5) * 1.02 / 25615, 3)`}   & `r round(1000 * quantile(resultado, probs = .5) * 1.04 / 25615, 3)`   \\ \hline
Remuestreo           & \multicolumn{1}{l|}{`r round(1000 * quantile(resultado_sampling, probs = .5) * 1.01 / 25615, 3)`}   & \multicolumn{1}{l|}{`r round(1000 * quantile(resultado_sampling, probs = .5) * 1.02 / 25615, 3)`}   & `r round(1000 * quantile(resultado_sampling, probs = .5) * 1.04 / 25615, 3)`   \\ \hline
...
Metodo     & \multicolumn{1}{l|}{1\%} & \multicolumn{1}{l|}{2\%} & 4\% \\ \hline
Weibull    & \multicolumn{1}{l|}{`r round(100 * (quantile(resultado, probs = .99) / (quantile(resultado, probs = .5) * 1.01) - 1), 2)`}   & \multicolumn{1}{l|}{`r round(100 * (quantile(resultado, probs = .99) / (quantile(resultado, probs = .5) * 1.02) - 1), 2)`}   & `r round(100 * (quantile(resultado, probs = .99) / (quantile(resultado, probs = .5) * 1.04) - 1), 2)`   \\ \hline
Remuestreo & \multicolumn{1}{l|}{`r round(100 * (quantile(resultado_sampling, probs = .99) / (quantile(resultado_sampling, probs = .5) * 1.01) - 1), 2)`}   & \multicolumn{1}{l|}{`r round(100 * (quantile(resultado_sampling, probs = .99) / (quantile(resultado_sampling, probs = .5) * 1.02) - 1), 2)`}   & `r round(100 * (quantile(resultado_sampling, probs = .99) / (quantile(resultado_sampling, probs = .5) * 1.04) - 1), 2)`   \\ \hline
\end{tabular}\caption{Margen de solvencia minimo en porcentaje de prima pura recargada}
\label{table:1}
\end{center}
\end{table}

Esta tabla muestra a la empresa el porcentaje de las primas puras recargadas que debe reservar como margen de solvencia minimo para garantizar su solvencia con una probabilidad del 99%, según la técnica empleada  y el porcentaje de recargo de seguridad.
